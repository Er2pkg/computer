export default (C, api, interaction) => {
	...
	case 2:
		...
		if (!cmd)
			interaction.createEphemeral(C.locale.get('error', 'inv_cmd', l))
		else if (typeof cmd.run != 'function')
			interaction.createEphemeral(C.locale.get('error', 'cmd_run', l))
		else if (cmd.private && !owner)
			interaction.createEphemeral(C.locale.get('error', 'adm_cmd', l))
		else {
			C.stats.uses++
			...
			.catch(err => {
				...
				interaction.createEphemeral(C.locale.get('error', 'not_suc', l))
			})
		}
		break
	}
